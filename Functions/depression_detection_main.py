...
# random_tweets_df      = random_tweets_df.sample(2204)
...
textfile = open("tweets_text_experiment2.txt", "w", encoding='utf8')
...
twee            = Dataset('tweets_text_experiment2.txt')
...
emb_tweets_df.to_csv('experiment2-emb.csv', index=False)
...
outfilename = 'experiment2-final-set.csv'
...
data                 = pd.read_csv('experiment2-final-set.csv')
data= data.dropna()
...
# 4- bert embeddings
bert                 = pd.read_csv('experiment2-emb.csv')
x       = bert.to_numpy()
y=d[:,11].astype('int')
